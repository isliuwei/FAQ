...
var server = http.createServer(function(req,res){
    ...
    var id = qs.parse(urlObj.query).id;
    if(pathname == '/'){
        switch(req.method){
            case 'GET':
                show(res);
...
var bitMap = function(){
    var bitArr = [];
    for(var i=0; i<=63; i++){
        bitArr[i] = parseInt(Math.random()*2);
    }
    return bitArr;
}
...
function show(res,logic,pageNo,blockNo,pageAdd,physical){
    if(!logic){
        logic = '未输入';
    }
    if(!pageNo){
...
<div class="container">
    <div class="row">
        <div class="col-md-12 jumbotron">
                <form role="form" method="post" class="col-md-4 form-inline" action="/">
                    <h3>逻辑地址</h3>
                    <div class="form-group">
                        <input type="number" class="form-control"  placeholder="(10进制数)" name="logic">
                    </div>
                    <button type="submit" class="btn btn-primary">转换</button>
                    <br>
                    <br>
                    <div>
                        <p><kbd>块长: <span>1024</span></kbd></p>
                        <p><kbd>逻辑地址: <span>${logic+"D"} | ${logic.toString(16)+"H"}</span></kbd></p>
                        <p><kbd>页内地址: <span>${pageAdd}</span></kbd></p>
                        <p><kbd>物理地址: <span>${physical+"D"} | ${physical.toString(16)+"H"}</span></kbd></p>
                        <p><kbd>FIFO缺页率: <span>${((countF/total)*100).toFixed(0)+"%"}</span></kbd></p>
                        <p><kbd>LRU缺页率: <span>${((countL/total)*100).toFixed(0)+"%"}</span></kbd></p>
                        <p><kbd>FIFO缺页数: <span>${countF}</span></kbd></p>
                        <p><kbd>LRU缺页数: <span>${countL}</span></kbd></p>
                        <p><kbd>TOTAL: <span>${total}</span></kbd></p>
                ...
                <div class="col-md-4">
                    <h3>页表</h3>
                    <ul class="list-group">
                        <li class="list-group-item mem list-group-item-use"><span>页号</span>|<span>块号</span>|<span>状态</span></li>
<li class="list-group-item mem list-group-item-success">${pagetable.map(function(item,index,array){var stat; if(item!=0){stat = 1}else{stat=0}return "<span>"+(index+1)+"</span>|<span>"+item+"</span>|<span>"+stat+"</span>"}).join('')}</li>
                ...
                <div class="col-md-4">
                    <h3>位示图</h3>
                    <div class="bitmap">
                    ${ bitmap.map(function(item,index,array){ var style; if(!item){style="byte"}else{style=""};return '<span class='+style+'>'+item+'</span>'}).join('')}
                    </div>
                </div>
        ...
        <div class="col-md-9 jumbotron">
            <div class="col-md-4 show">
                <h2>FIFO</h2>
                <div class="bitmapF">
                ${ stackF.map(function(item,index,array){ return '<span>'+item+'</span>'}).join('')}
            ...
            <div class="col-md-4 col-md-offset-2 show">
                <h2>LRU</h2>
                <div class="bitmapL">

                ${ stackL.map(function(item,index,array){ return '<span>'+item+'</span>'}).join('')}
                </div>
...
var convert = function(req,res)
{
    ...
    req.on('end',function(){
        ...
        var logic = qs.parse(body).logic;
        var pageNo = parseInt(logic / 1024);
        var pageAdd = logic % 1024;
        var blockNo;
        var physical;
        ...
        {
            blockNo = pagetable[pageNo-1];
            physical = (blockNo+1)*1024 + pageAdd;
            arr.push(pageNo);
            FIFO(pageNo);
            LRU(pageNo);
            total++;
        }else{
            blockNo = bitmap.indexOf(0);
            if(pagetable.length<len){
                if(blockNo!=-1){
                    pagetable[pageNo-1] = blockNo+1;
                    physical = (blockNo+1)*1024 + pageAdd;
                    bitmap[blockNo] = 1;
                    arr.push(pageNo);
                    FIFO(pageNo);
        ...
        show(res,logic,pageNo,blockNo,pageAdd,physical);
...
function FIFO(num) {
    flag = true;
    var lab = true;
    if(stackF.length>=len){
        for(var j = 0; j < stackF.length ;j++){
            if(num == stackF[j]){
                flag = !flag;
                break;
            }
...
function LRU(num) {
    flag = true;
    var lab = true;;
    if(stackL.length>=len){
        for(var j = 0; j < stackL.length ;j++){
            if(num == stackL[j]){
                stackL.splice(j,1);
                stackL.push(num);
                flag = !flag;
